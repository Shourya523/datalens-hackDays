...
import { Loader2, AlertTriangle, CheckCircle2, BarChart3, Sparkles, Database } from "lucide-react";

const FALLBACK_URI = process.env.NEXT_PUBLIC_FALLBACK_URI!;

export default function QualityDynamicPage({ params }: { params: Promise<{ connectionId: string }> }) {
  ...
  useEffect(() => {
    const fetchConns = async () => {
      ...
      setConnections([demoConn, ...userConns.filter(c => c.id !== DEMO_ID)]);
    };
    fetchConns();
  }, [session]);
  ...
  // Fetch tables when connectionId or connections list updates
  useEffect(() => {
    const fetchTables = async () => {
      if (!connectionId || authLoading) return;
      ...
      try {
        const uri = await getEffectiveUri(connectionId);
        if (!uri) return;
    ...
    fetchTables();
  }, [connectionId, session, authLoading, connections.length]);

  const handleConnectionChange = (newConnId: string) => {
    if (!newConnId) return;
    router.push(`/dashboard/quality/${encodeURIComponent(newConnId)}`);
  };

  const runAudit = async () => {
    const tableObj = tables.find(t => t.name === selectedTable);
    if (!connectionId || !tableObj) return;
