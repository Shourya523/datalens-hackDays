...
import { getUserById } from "@/src/actions/auth";
...
export default function DashboardOverview() {
  ...
  // Get username from localStorage (from signup) or session
  ...
  useEffect(() => {
    async function fetchUserData() {
      ...
      if (userData) {
        ...
        setUsername(parsed.username || parsed.name?.split(" ")[0] || "Explorer");
      } else if (session?.user?.id) {
        // Fetch user data from database
        const dbUser = await getUserById(session.user.id);
        if (dbUser) {
          setUsername(dbUser.username || dbUser.name?.split(" ")[0] || "Explorer");
  ...
  const quickActions = [
...

    {
      ...
      href: lastConnId ? `/dashboard/quality/${lastConnId}` : "/dashboard/quality",
...

    {
      ...
      href: lastConnId ? `/dashboard/lineage/${lastConnId}` : "/dashboard/lineage",
